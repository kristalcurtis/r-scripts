## 6.22.10
# Use cross validation to choose the proper # of mixture components for general mixture models
# Mixture model is fitted to data via EM -- see "emForGeneralMixtureModel.R"
...
fitMMToTrainingDataAndReturnValidationLikelihood = function(trainingDataVector, validationDataVector, numMixtureComponents, distrType, initType) {
...
#	}
	...
	# fit MM to training data
	emFit = em(trainingDataVector, numMixtureComponents, distrType, initType)
	
	# get loglik of validation data under MM
	validationLoglik = computeLoglik(validationDataVector, emFit$params, distrType)
	
	# return validation loglik
	return(validationLoglik)
}



findAvgLogLikelihoodViaCVFixedNumMixtureComponents = function(dataVector, numFolds, numMixtureComponents, distrType, initType, destinationPath="") {
	...
	for (i in 1:numFolds) {
		...
		logLikelihood[i] = fitMMToTrainingDataAndReturnValidationLikelihood(trainingData, validationData, numMixtureComponents, distrType, initType)
...
kFoldCVToChooseNumMixtureComponentsForMM = function(dataVector, distrType, initType, destinationPath, numFolds=10, mixtureComponentsRange=seq(from=2,by=1,to=10)) {
	...
	for (i in 1:length(mixtureComponentsRange)) {
		...
		avgLogLikelihood[i] = findAvgLogLikelihoodViaCVFixedNumMixtureComponents(dataVector, numFolds, mixtureComponentsRange[i], distrType, initType, destinationPath)
	}
...
plotCVLogLikelihoodVsNumMixtureComponents = function(path, mixtureComponentsRange=seq(from=2,by=1,to=10)) {
	...
	legend("bottomright", legend=c("log likelihood per fold", "average", "median"), col=c("blue", "red", "green"), pch=c(1,-1,-1), lty=c(0,1,1), lwd=c(2,2,2))
